...
- **Accept a scoped exception** — only with explicit governance authorization from the principal, with a sunset condition.
...
- **Audit reveals a scoped CSP exception is needed** (rare) — escalate to the principal for governance authorization.