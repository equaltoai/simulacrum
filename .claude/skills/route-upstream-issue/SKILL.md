...
If the upstream genuinely blocks critical sim work, escalate to the principal. Forking / patching is scope-level, not steward-level.
...
- **"Fork FaceTheory and maintain a sim-specific version."** Refuse. Escalate to the principal if the framework is genuinely blocking.
...
- **Upstream refuses / deprioritizes** — escalate to the principal for decision (accept permanent workaround, contribute the upstream fix ourselves, or rework sim's approach).
