<script lang="ts">
	...
	const visibleNotes = $derived(showAll ? notes : notes.slice(0, maxInitialNotes));
	const hasMoreNotes = $derived(notes.length > maxInitialNotes);
...
{#if hasNotes}
	<div class={`status-community-notes ${className}`} role="complementary" aria-label="Community Notes">
		<div class="community-notes__header">
			...
			<span class="community-notes__count">{notes.length}</span>
		</div>
			...
			{#each visibleNotes as communityNote (communityNote.id)}
				<div class="community-note">
					...
					{:else}
						...
						<div class="community-note__content">{communityNote.content}</div>
		...
		{#if hasMoreNotes}
			<button class="community-notes__toggle" onclick={toggleShowAll} type="button">
				{showAll ? 'Show fewer notes' : `Show ${notes.length - maxInitialNotes} more notes`}
			</button>
		{/if}
	</div>
{/if}
