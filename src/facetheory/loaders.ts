...
function buildRuntimeBoundary(
...
): FaceAgentRuntimeBoundaryData {
	const agent = activeAgent ?? PREVIEW_AGENT;
	const semantics = workflow?.identitySemantics ?? activeAgent?.identitySemantics ?? PREVIEW_AGENT.identitySemantics;
	const trimmedExpectedWallet = expectedWallet?.trim() ?? null;
	const walletState =
		semantics.identityState === 'souled'
			? 'available'
			: trimmedExpectedWallet
				? 'restricted'
				: 'locked';
	...
	return {
		title:
			semantics.identityState === 'souled'
				? 'Published soul runtime'
				: semantics.identityState === 'graduating'
					? 'Graduating runtime'
					: 'Drone runtime boundary',
		summary:
			semantics.identityState === 'souled'
				? 'This body now operates as the published soul while preserving the existing continuity trail.'
				: 'This body stays in the lightweight drone tier until graduation completes, so runtime access remains intentionally constrained.',
		moderationLabel: semantics.moderationLabel,
		attributionLabel: semantics.attributionLabel,
		continuitySummary: semantics.continuitySummary,
		expectedWallet: trimmedExpectedWallet ?? undefined,
		capabilities: [
			{
				id: 'runtime-post',
				label: 'Posting',
				state: agent.agentCapabilities.canPost ? 'available' : 'restricted',
				detail: agent.agentCapabilities.canPost
					? `Body may publish up to ${agent.agentCapabilities.maxPostsPerHour} post(s) per hour.`
					: 'Posting is currently withheld from this runtime tier.',
			},
			{
				id: 'runtime-reply',
				label: 'Replies',
				state: agent.agentCapabilities.canReply ? 'available' : 'restricted',
				detail: agent.agentCapabilities.canReply
					? 'Replies remain available inside the delegated drone runtime.'
					: 'Reply permissions are withheld until the body is promoted.',
			},
			{
				id: 'runtime-follow',
				label: 'Follows',
				state: agent.agentCapabilities.canFollow ? 'available' : 'restricted',
				detail: agent.agentCapabilities.canFollow
					? 'Follow actions are available within the current delegated policy.'
					: 'Follow actions are currently restricted for this body.',
			},
			{
				id: 'runtime-comms',
				label: 'Direct communication',
				state: agent.agentCapabilities.canDM ? 'available' : 'locked',
				detail: agent.agentCapabilities.canDM
					? 'Direct communication is available on the current soul-facing runtime.'
					: 'Direct communication stays locked until graduation unlocks the soul runtime.',
			},
			{
				id: 'runtime-wallet',
				...
				detail:
					walletState === 'available'
						? `Registered wallet ${truncateWallet(trimmedExpectedWallet ?? '')} is active for the published soul.`
						: walletState === 'restricted'
							? `Registered wallet ${truncateWallet(trimmedExpectedWallet ?? '')} is reserved for finalize and cannot act as a general runtime surface yet.`
							: 'No wallet surface is available on this body before finalize.',
			},
			{
				id: 'runtime-mcp',
				label: 'MCP workspace',
				state: agent.mcpAccess.scopes.length ? 'available' : 'restricted',
				detail: agent.mcpAccess.scopes.length
					? `${agent.mcpAccess.scopes.length} scope(s) are delegated through the MCP runtime.`
					: 'No delegated MCP scopes are currently active for this body.',
			},
		],
		delegatedScopes: agent.delegatedScopes.length ? [...agent.delegatedScopes] : undefined,
		mcpGuidance: agent.mcpAccess.guidance.length ? [...agent.mcpAccess.guidance] : undefined,
		mcpUrl: agent.mcpAccess.mcpURL,
	};
}
	...
	lifecycleEvents: [
		{
			...
			promotion: {
				...
				next_actions: ['Collect principal wallet signature'],
			},
		},
	],
	conversations: [
...
function assembleAppState(input: AssembleAppStateInput): ClientAppState {
	...
	const runtimeBoundary = buildRuntimeBoundary(activeAgent, workflow, expectedWallet);
